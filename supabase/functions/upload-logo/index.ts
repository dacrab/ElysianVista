...
app.post('/upload-logo', async (c) => {
  try {
    const supabaseClient = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_ANON_KEY, {
    ...
    const { data, error } = await supabaseClient.storage.from('logos').upload(filePath, file);
    ...
    const {
      data: { publicUrl },
    } = supabaseClient.storage.from('logos').getPublicUrl(data.path);
